...
function TransequipmentTable({ ...props }) {
  ...
  const handleCreateNewRow = async (values) => {
    ...
    values.code = code;
    const data = await addTransequipment(values);
    var today = new Date();
    const value = data.data.addTransequipment;
  ...
  const handleDeleteRow = useCallback(
    async (row) => {
      ...
      const data = await deleteTransequipment({ id: id });
      handleOpen({
        color: "#009933",
...
export default TransequipmentTable;
