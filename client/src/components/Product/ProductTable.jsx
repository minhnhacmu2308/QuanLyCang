...
function ProductTable({ ...props }) {
  ...
  const [tableData, setTableData] = useState(() => props.props.data.data.Products);
  const [units, setUnits] = useState(() => props.props.units.data.units);
  const [categorys, setCategorys] = useState(() => props.props.categorys.data.categorys);
  ...
  const handleCreateNewRow = async (values,unitId,categoryId) => {
    ...
    var today = new Date();
    const value = data.data.addProduct;
  ...
  const data = await updateProduct(values);
   ...
   tableData[index] = data.data.updateProduct;
...
export const CreateNewAccountModal = ({
  ...
  const handleSubmit = (e) => {
    console.log("values",values)
    if (!!values.color && !!values.size && !!values.name && !!unitId && unitId !=="default" && !!categoryId && categoryId !=="default" && values.name !== "default"
    && values.color !== "default" && values.size !== "default") {
...
export const UpdateAccountModal = ({
  ...
  useEffect(() => {
    ...
    fields["unitId"] = dataUpdate.unit?._id;
    fields["categoryId"] = dataUpdate.category?._id;
    fields["nameUnit"] = dataUpdate.unit?.name;
    fields["nameCategory"] = dataUpdate.category?.name;
  ...
  },[dataUpdate.name,dataUpdate.unit?._id]);
  const handleSubmit = (e) => {
    if (handleValidation()) {
      onSubmit(fields)
    } return;
      ...
      <DialogContent>
        <form onSubmit={(e) => e.preventDefault()}>
          ...
          >
              ...
              <select style={{height:55}} class="form-select" onChange={(e)=>handleChange(e)} name="unitId" required>
                  <option value={fields["unitId"]} selected>{fields["nameUnit"]}</option>
                  {units.map((option) => (
            ...
            <select style={{height:55}} class="form-select" onChange={(e)=>handleChange(e)} name="cateogoryId" required>
                  <option value={fields["categoryId"]} selected>{fields["nameCategory"]}</option>
                  {categorys.map((option) => (
                        <option value={option._id}>{option.name}</option>
                    ))}
